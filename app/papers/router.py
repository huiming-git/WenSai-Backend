...
from app.papers.schemas import PaperCreate, PaperFinalize, PaperListResponse, PaperResponse, PaperUpdate
...
def _delete_paper_file_from_storage(file_path: str | None) -> None:
...
def delete_paper(
    ...
    _delete_paper_file_from_storage(paper.file_path)
    db.delete(paper)
    db.commit()
...
async def upload_file(
    ...
    try:
        ...
        previous_file_path = paper.file_path
        paper.file_path = key
        db.commit()
        db.refresh(paper)
        if previous_file_path and previous_file_path != key:
            _delete_paper_file_from_storage(previous_file_path)
        return paper
    finally:
        await file.close()
